...
export function useTargets() {
  ...
  const batchSetStatus = useCallback((targetIds: string[], status: TargetStatus) => {
    if (targetIds.length === 0) return;
    commitTargetGraphMutation((state) => ({
      ...state,
      targets: state.targets.map((target) =>
        targetIds.includes(target.id) ? { ...target, status, updatedAt: Date.now() } : target,
      ),
    }));
  }, []);

  const batchAddToGroup = useCallback((targetIds: string[], groupId: string) => {
    if (targetIds.length === 0) return;
    const store = useTargetGroupStore.getState();
  ...
  const batchAddTags = useCallback((targetIds: string[], newTags: string[]) => {
    if (targetIds.length === 0 || newTags.length === 0) return;
    commitTargetGraphMutation((state) => ({
      ...state,
      targets: state.targets.map((target) => {
        if (!targetIds.includes(target.id)) return target;
        const merged = uniqueStrings([...target.tags, ...newTags]);
        if (merged.length === target.tags.length) return target;
        return { ...target, tags: merged, updatedAt: Date.now() };
      }),
    }));
  }, []);

  const batchSetCategory = useCallback((targetIds: string[], category: string) => {
    if (targetIds.length === 0) return;
    commitTargetGraphMutation((state) => ({
      ...state,
      targets: state.targets.map((target) =>
        targetIds.includes(target.id) ? { ...target, category, updatedAt: Date.now() } : target,
      ),
    }));
  }, []);
